...
async function claim() {
  ...
  const claimer = new PublicKey("7q66w6j8oWnctRjkxDFuP6h5YU3LpsKbDUAtpf7eTnRo");
  const MINT = new PublicKey("CJJfJQThGGYiTX4Lhyo4C2qvHZWnXGUWRgnMn9U54bVV");

  const buyer_account = getBuyerAccount(claimer.toBuffer());
  const box_account = getBoxAccount(4);

  const nft_box = await getAssociatedTokenAddress(MINT, box_account);
  console.log("NFT BOX:", nft_box);
  ...
  console.log(buyer_account_info.boughts[0].id.toString());
